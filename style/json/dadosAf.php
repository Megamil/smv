...
//Usado pelo JSon.sj Para a tela de Nova Solicitação Ordem de Serviço e a Edição da Solicitação;
	...
	if(!$conn) {
		...
		$sql 	= "select tca.dtinivigencia,tca.dtfimvigencia,prazoentrega,prazopagto,numprocadmempenho,numprocregpreco, objetotexto objeto from tbl_contratoata tca 
left join tbl_objeto on id_objeto = id_objetotitulo
left join tbl_empenho on numcontratoata = id_contratoata where id_contratoata ='".$_GET["id"]."';"; 
