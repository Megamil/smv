...
//Usado pelo JSon.sj Para a tela de Nova Solicitação Ordem de Serviço e a Edição da Solicitação;
	...
	if(!$conn) {
		...
		$sql 	= "select prefixo from tbl_ordemservico left join
tbl_solicitaordemservico ts on id_solicitaordemservico = id_solicitacao
left join tbl_veiculo tv on tv.id_veiculo = ts.id_veiculo
where id_ordemservico = '".$_GET["id"]."';"; 
